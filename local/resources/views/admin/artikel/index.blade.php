          ...
          <div class="">
              ...
              <div class="title_right">
                <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search">
                  <div class="input-group">
                     ...
                     {!! Form::open(['url' => 'admin/artikel/search', 'files' => true, 'style' => 'display:inherit;']) !!}
                     {!! Form::text('search', null, ['class' => 'form-control', 'placeholder' => 'Cari Artikel...', 'style' => 'border-radius:25px 0px 0px 25px;']) !!}<br>
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="x_panel">
                  <div class="x_title">
                    <a href="{{ url('admin/artikel/tambah_baru') }}">
                      <button type="button" class="btn btn-primary"><i class="fa fa-pencil">  Artikel Baru</i></button>
                    </a>
                      ...
                      <?php $x=1; ?>
                       @foreach($kategori_list as $kategori)
                        <tr>
                          <td><?php echo $x; $x++; ?></td>
                          <td>
                            <a>{{ $kategori->judul_artikel }}</a>
                            <br />
                            <small>Dibuat pada {{ $kategori->created_at }}</small>
                          </td>
                          <td >
                            <a>{!! strip_tags(str_limit($kategori->isi, $limit = 100, $end = '.......')) !!}</a>
                          </td>
                          <td>
                            @if(count($kategori->id_kategori)>0)
                             <a>{{ $kategori->kategori->nama_kategori }}</a>
                            @else
                              <a>Tidak Memiliki Kategori</a>
                            @endif
                            <!-- <button type="button" class="btn btn-success btn-xs">Aktif</button> -->
                          </td>
                          <td>
                            <a target="_blank" href="{{ url('artikel/'.$kategori->id) }}" class="btn btn-primary btn-xs"><i class="fa fa-eye"></i> Lihat </a>
                            <a href="{{ url('admin/artikel/'.$kategori->id.'/edit') }}" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Ubah </a>
                           <button class="delete-artikel btn btn-danger btn-xs" data-photo-id="{{$kategori->id}}">Hapus</button>
                          </td>
                        </tr>
                       @endforeach
                      </tbody>
